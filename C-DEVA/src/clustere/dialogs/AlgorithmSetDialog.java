...
public class AlgorithmSetDialog extends org.eclipse.swt.widgets.Dialog {
	...
	public void open() {
		try {
			...
			dialogShell.setText("Parameter Set");
...
//					"icos/open.ico"));
			...
			for (int i = 0; i < str.length; i++) {
				...
				if(value[i].equals("true") || value[i].equals("false")){
					Button cBox = new Button(dialogShell, SWT.CHECK);
					cBox.setText(str[i]);
					cBox.setSelection(Boolean.valueOf(value[i]));
					vector.add(cBox);				
			...
			confirm.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					...
					for (int i = 0; i < result.length; i++) {
						if(vector.get(i) instanceof Text){
							result[i] = ((Text)vector.get(i)).getText();
						}else if(vector.get(i) instanceof Button){
							if(((Button)vector.get(i)).getSelection()){
								result[i] = "true";
							}else{
								result[i] = "false";
							}
