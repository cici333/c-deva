...
//	public Map<String,Integer> edgeMap = new HashMap<String,Integer>();
	...
	public void readClusters(Vector<Node>[] cluster) throws IOException{
		...
		for(int i=0;i<cluster.length;i++){
			...
			c.setClusterid(String.valueOf(i+1));
			clusters.add(c);
...
//		double min = 100;  //p-value最小值初始化
		...
		String fmeasurefun = "", hfmeasurefun = "";
		int fmeasurebackground = 0, hfmeasurebackground = 0,fmeasurefunnum = 0, hfmeasurefunnum = 0;
		
		HashMap<String, Double> wmap = new HashMap<String, Double>();
		for(ClusterVo cluster :clusters){//依次获取各个cluster对象
			...
			while(it2.hasNext()){  //依次遍历每个功能  计算p-value值最小的那一个
				...
				if(f_measure>fmeasure){
					fmeasure = f_measure;
					fmeasurefun = fun;
					fmeasurebackground = background;
					fmeasurefunnum = funnum;
...
//				System.out.println(fun+"\t\t"+hf_measure+"\t"+total_wi);
				if(hf_measure>hfmeasure){
					...
					hfmeasurefun = fun;
					hfmeasurebackground = background;
					hfmeasurefunnum = funnum;
			...
			cluster.setHfmeasureTb(hfmeasure);
			cluster.setFmeasureFun(fmeasurefun);
			cluster.setBackground(fmeasurebackground);
			cluster.setFunnum(fmeasurefunnum);
			cluster.setHfmeasureFun(hfmeasurefun);
			cluster.setHbackground(hfmeasurebackground);
			cluster.setHfunnum(hfmeasurefunnum);
			
	//		System.out.println("&&&&&&&&&&&&"+hfmeasure+"&&&&&&&&&&&&&&");
...
//				System.out.println(fun+"\t\t"+hf_measure+"\t"+total_wi);
			...
			cluster.setFmeasure(fmeasure);
			cluster.setHfmeasureTf(hfmeasure);
			cluster.setFmeasureFun(fmeasurefun);
			cluster.setBackground(fmeasurebackground);
			cluster.setFunnum(fmeasurefunnum);
			cluster.setHfmeasureFun(hfmeasurefun);
	...
	public void setClusters(List<ClusterVo> list){
		this.clusters = list;
	}
}
